...
class ComboTalonSRX:
    ...
    def setup(self):
        mainMotor = ctre.TalonSRX(self.canID_leader)
        mainMotor.setInverted(self.inverted)
...
class DriveTrainModule:
    ...
    def execute(self):
        ...
        if self.is_leftChanged():
            self.mainLeft_motor.set(self.leftSpeed)
            self.leftSpeedChanged = False
        else:
            pass

        if self.is_rightChanged():
            self.mainRight_motor.set(self.rightSpeed)
            self.rightSpeedChanged = False
        else:
            pass